...
folder_id = "123xBZ2YDy8BZrbErQb0U9TpGY-j3NdK7"  # Google Drive folder ID
...
# Display previous conversation on rerun
...
# Initialize conversation only if messages is empty
if not st.session_state.messages:
    ...
    # Do not call save_interview_data here to avoid writing an incomplete transcript.
    ...
    if message_respondent := st.chat_input("Your message here"):
        ...
        with st.chat_message("assistant", avatar=config.AVATAR_INTERVIEWER):
            ...
            if not any(code in message_interviewer for code in config.CLOSING_MESSAGES.keys()):
                ...
                st.session_state.messages.append({"role": "assistant", "content": message_interviewer})
                try:
                    # Do not call save_interview_data here repeatedly.
                    transcript_link, transcript_file = save_interview_data(
                        folder_id=folder_id,
                        student_number=query_params["student_number"],
                        company_name=query_params["company"]
